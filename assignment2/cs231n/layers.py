...
def flip(x):
    ...
    # Column flip
        for i in xrange(len/2):
            temp1=np.array(x[c,:,len-i])
            x[c,:,len-i]=x[c,:,i]
            x[c,:,i]=temp1
    # Row flip
        for j in xrange(len/2):
            temp2=np.array(x[c,len-j,:])
            x[c,len-j,:]=x[c,j,:]
            x[c,j,:]=temp2
    return x
...
def dropout_forward(x, dropout_param):
  ...
  if mode == 'train':
    ...
    ###########################################################################
    boolean=np.random.random(size=x.shape)
    mask=((boolean>dropout_param['p'])*1)/dropout_param['p']
    out=(x*mask)
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
...
def conv_forward_naive(x, w, b, conv_param):
  ...
  W=x.shape[3]
  C=x.shape[1]
  H_tilde = 1 + (H + 2 * pad - HH) / stride
  W_tilde = 1 + (W + 2 * pad - WW) / stride
  out=np.ones(shape=(N,F,H_tilde,W_tilde))
  #############################################################################
  ...
  for n in range(N):
      for f in range(F):
          for i,k in zip(range(0,H_tilde+1,stride),range(H_tilde)):
              if(i+HH>x_pad.shape[2]):
                  break
              for j,l in zip(range(0,W_tilde+1,stride),range(W_tilde)):
                  if(j+WW>x_pad.shape[3]):
                      break
                  out[n,f,k,l]=np.sum(x_pad[n,:,i:HH+i,j:WW+j]*w[f,:,:,:])+b[f]
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  cache = (x, w, b, conv_param)
...
def max_pool_forward_naive(x, pool_param):
  ...
  N,C,H,W=x.shape
  out=np.zeros(shape=(N,C,H/h,W/w))
  #############################################################################
  ...
  for n in xrange(N):
      for c in xrange(C):
          for i in xrange(H/h):
              ...
              for j in xrange(W/w):
                  ws=j*stride
                  out[n,c,i,j]=np.max(x[n,c,hs:hs+h,ws:ws+w])
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  cache = (x, pool_param)
...
def max_pool_backward_naive(dout, cache):
  ...
  for n in xrange(N):
      for c in xrange(C):
          for i in xrange(H/HH):
               hs=i*stride
               for j in xrange(W/WW):
                   ws=j*stride
                   p,q=np.unravel_index(np.argmax(x[n,c,hs:hs+HH,ws:ws+WW]),x[n,c,hs:hs+HH,ws:ws+WW].shape)
                   k=(n,c,p+hs,q+ws)
                   dx2[k]=1
                   dx[n,c,hs:hs+HH,ws:ws+WW]+=dx2[n,c,hs:hs+HH,ws:ws+WW]*dout[n,c,i,j]
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  return dx
